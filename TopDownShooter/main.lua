function love.load()
    math.randomseed(os.time())
    sprites = {}
    sprites.background = love.graphics.newImage('sprites/background.png')
    sprites.bullet = love.graphics.newImage('sprites/bullet.png')
    sprites.player = love.graphics.newImage('sprites/player.png')
    sprites.zombie = love.graphics.newImage('sprites/zombie.png')

    myFont = love.graphics.newFont(30)

    player = {}
    player.x = love.graphics.getWidth() / 2
    player.y = love.graphics.getHeight() / 2
    player.speed = 300
    player.extraLives = 1
    player.injured = false

    zombies = {}
    zombies.speed = 60
    player.injuredSpeed = 270
    bullets = {}

    gameState = 1
    maxTime = 2
    timer = maxTime
    score = 0
end

function love.update(dt)
    if gameState == 2 then
        local moveSpeed = player.speed
        if player.injured then
            moveSpeed = player.injuredSpeed
        end
        if love.keyboard.isDown("w") and player.y >= sprites.player:getHeight() / 2 then
            player.y = player.y - dt * moveSpeed
        end
        if love.keyboard.isDown("a") and player.x >= sprites.player:getWidth() / 2 then
            player.x = player.x - dt * moveSpeed
        end
        if love.keyboard.isDown("s") and player.y <= love.graphics.getHeight() - sprites.player:getHeight() / 2 then
            player.y = player.y + dt * moveSpeed
        end
        if love.keyboard.isDown("d") and player.x <= love.graphics.getWidth() - sprites.player:getWidth() / 2 then
            player.x = player.x + dt * moveSpeed
        end
    end


    for i, z in ipairs(zombies) do
        z.x = z.x + math.cos(zombiePlayerAngle(z)) * dt * zombies.speed
        z.y = z.y + math.sin(zombiePlayerAngle(z)) * dt * zombies.speed

        if distanceBetween(z.x, z.y, player.x, player.y) < 30 then
            if player.extraLives > 0 then
                player.extraLives = player.extraLives - 1
                player.injured = true
                z.dead = true
            else
                for i, z in ipairs(zombies) do
                    zombies[i] = nil
                end
                gameState = 1
                player.x = love.graphics.getWidth() / 2
                player.y = love.graphics.getHeight() / 2
                player.injured = false
            end
        end
    end

    for i, b in ipairs(bullets) do
        b.x = b.x + math.cos(b.direction) * dt * b.speed
        b.y = b.y + math.sin(b.direction) * dt * b.speed
        -- if distanceBetween(b.x, b.y, player.x, player.y) < 30 then
        --     for i, b in ipairs(zombies) do
        --         zombies[i] = nil
        --     end
        -- end
    end

    for i = #bullets, 1, -1 do
        local b = bullets[i]
        if b.x < 0 or b.x > love.graphics.getWidth() or b.y < 0 or b.y > love.graphics.getHeight() then
            table.remove(bullets, i)
        end
    end

    for i, z in ipairs(zombies) do
        for j, b in ipairs(bullets) do
            if distanceBetween(z.x, z.y, b.x, b.y) < 20 then
                z.dead = true
                b.dead = true
                score = score + 1
            end
        end
    end

    for i = #zombies, 1, -1 do
        local z = zombies[i]
        if z.dead then
            table.remove(zombies, i)
        end
    end

    for i = #bullets, 1, -1 do
        local b = bullets[i]
        if b.dead then
            table.remove(bullets, i)
        end
    end

    if gameState == 2 then
        timer = timer - dt
        if timer <= 0 then
            spawnZombie()
            maxTime = 0.98 * maxTime
            timer = maxTime
        end
    end
end

function love.draw()
    love.graphics.setColor(1, 1, 1)
    love.graphics.draw(sprites.background, 0, 0)
    -- love.graphics.print("ph:" .. sprites.player:getHeight(), 0, 0)
    -- love.graphics.print("pw:" .. sprites.player:getWidth(), 100, 0)
    -- love.graphics.print("lh:" .. love.graphics.getHeight(), 300, 0)
    if gameState == 1 then
        love.graphics.setFont(myFont)
        love.graphics.printf("Click anywhere to begin", 0, 50, love.graphics.getWidth(), "center")
    end

    love.graphics.printf("Score: " .. score, 0, love.graphics.getHeight() - 100, love.graphics.getWidth(), "center")

    if player.injured then
        love.graphics.setColor(1, 0, 0)
    end
    love.graphics.draw(sprites.player, player.x, player.y, playerMouseAngle(), nil, nil, sprites.player:getWidth() / 2,
        sprites.player:getHeight() / 2)
        for i, z in ipairs(zombies) do
        love.graphics.setColor(z.colorR, z.colorG, z.colorB)
        love.graphics.draw(sprites.zombie, z.x, z.y, zombiePlayerAngle(z), nil, nil, sprites.zombie:getWidth() / 2,
            sprites.zombie:getHeight() / 2)
    end
    for i, b in ipairs(bullets) do
        love.graphics.setColor(b.colorR, b.colorG, b.colorB)
        love.graphics.draw(sprites.bullet, b.x, b.y, nil, 0.5, nil, sprites.bullet:getWidth() / 2,
            sprites.bullet:getHeight() / 2)
    end
end

function playerMouseAngle()
    return math.atan2(love.mouse.getY() - player.y, love.mouse.getX() - player.x)
end

function zombiePlayerAngle(enemy)
    return math.atan2(player.y - enemy.y, player.x - enemy.x)
end

function spawnZombie()
    local zombie = {}

    zombie.x = 0
    zombie.y = 0
    zombie.speed = 100
    zombie.colorR = math.random()
    zombie.colorG = math.random()
    zombie.colorB = math.random()
    zombie.dead = false
    --[[
        x is left to right
        y is top to bottom

                  2
                y=-30
      _______________________
  1  |                       |      3
x=-30|                       | x=getWidth()+30
     |                       |
     |                       |
     ~~~~~~~~~~~~~~~~~~~~~~~~~
        y=getHeight()+30
                 4

    ]]
    local side = math.random(1, 4)
    if side == 1 then --Left
        zombie.x = -30
        zombie.y = math.random(0, love.graphics.getHeight())
    elseif side == 2 then --Top
        zombie.x = math.random(0, love.graphics.getWidth())
        zombie.y = -30
    elseif side == 3 then --Right
        zombie.x = love.graphics.getWidth() + 30
        zombie.y = math.random(0, love.graphics.getHeight())
    elseif side == 4 then
        zombie.x = math.random(0, love.graphics.getWidth())
        zombie.y = love.graphics.getHeight() + 30
    end

    table.insert(zombies, zombie)
end

function spawnBullet()
    local bullet = {}
    bullet.x = player.x
    bullet.y = player.y
    bullet.dead = false
    bullet.speed = 500
    bullet.direction = playerMouseAngle()
    bullet.colorR = math.random()
    bullet.colorG = math.random()
    bullet.colorB = math.random()
    table.insert(bullets, bullet)
end

function love.keypressed(key)
    if key == "space" then
        spawnZombie()
    end
end

function love.mousepressed(x, y, button)
    if button == 1 and gameState == 2 then
        spawnBullet()
    elseif button == 1 and gameState == 1 then
        gameState = 2
        maxTime = 2
        timer = maxTime
        score = 0
        player.injured = false
        player.extraLives = 1
    end
end

function distanceBetween(x1, y1, x2, y2)
    return math.sqrt((x2 - x1) ^ 2 + (y2 - y1) ^ 2)
end
